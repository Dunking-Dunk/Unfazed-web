    ...
    return (
        <div className="fixed w-full h-20 z-50 flex items-center lg:px-14  px-8 justify-between lg:backdrop-blur-md">
            <div className='lg:w-fit w-full flex items-center justify-center'>
                <Image src={unfazedLogo} alt='unfazed logo' className='lg:relative w-56 cursorOver' onClick={() => router.push('/')} />
            </div>
            <div className={`flex lg:flex-row lg:items-center justify-evenly lg:w-1/3 lg:h-full max-lg:fixed  flex-col h-screen w-full left-0 top-0 ${open ? 'top-0' : 'top-[-150%]'} lg:text-sm text-6xl lg:bg-transparent  z-50 md:p-20 p-10 lg:p-0 transition-all ease-out duration-200 lg:backdrop-blur-lg backdrop-blur-3xl text-[var(--primary-color)]`} ref={navbarRef}>
                <Link href='/about' className='cursorOver'>About</Link>
                <Link href='/work' className='cursorOver'>Works</Link>
            </div>
